...
# ============================================================================
FILTER_INTERVALS = ["1d"]  # change to None to process all intervals
FILTER_SYMBOLS = None
# FILTER_SYMBOLS   = ['MRNA', 'GILD', 'JNJ', 'NEM', 'HD', 'AAPL', 'MSFT', 'GOOGl', 'AVGO', 'ORCL', 'INTU', 'TSM', 'SAP', 'RTX', 'LMT', 'HON', 'TMO', 'DHR', 'PGR',
# 'TRV', 'CB', 'UNP', 'ITW', 'PH', 'ORLY', 'TJX', 'AZO', 'AVB', 'CPT']     
...
# FILTER_SYMBOLS = ["AIZ", "AMD", "APP", "ARES", "AXON", "CEG", "CMI", "CRWD", "CTRA", "DDOG", "DLR", "DY",
#     "KLAC",
#     "CRWD",
#     "AXON",
#     "CMI",
...
#     "VST",
#     "OXY",
#     "FIGR", "AMD", "PANW", "PLTR", "APP", "DDOG"]  # new - 10April April2026 - commpleted Gridsearch 1h - no results
...
buy_type = [
    ...
    "trend_reverse",
    "trend_reverse_long",
    "trend",            # FIX: re-added — was missing vs William's version
    "trend_reverse_short",
    "mr",
    "mr_reverse",
]
...
N_JOBS = -1  # FIX: use all available CPU cores (optimal for RunPod)
...
if __name__ == "__main__":
    ...
    def parse_filename(fname):
        ...
        print(f"  WARNING: Could not parse '{fname}' (expected merged_SOURCE_SYMBOL_INTERVAL.csv) — skipping")
        return None, None
    ...
    for csv_file in csv_files:
        ...
        INTERVAL_TO_PERIOD = {
            "1min":  120960,  # 252 * 8 * 60 — matches util_AQS_parallel.py
            "5min":  24192,   # 252 * 8 * 12 — matches util_AQS_parallel.py
            "15min": 8064,    # 252 * 8 * 4  — matches util_AQS_parallel.py
            "30min": 4032,    # 252 * 8 * 2  — matches util_AQS_parallel.py
            "1h":    2016,    # 252 * 8      — matches util_AQS_parallel.py
        ...
        for col in remaining_columns:
            ...
            for models in model:
                ...
                for buy_types in buy_type:
                    ...
                    is_results_list = Parallel(n_jobs=N_JOBS, verbose=0)(  # FIX: was verbose=10, too noisy
